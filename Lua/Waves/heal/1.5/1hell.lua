...
if (spawntime%50==25 and spawntime<211) then
	rand=math.random(2)*2-3
	local dist=128/6
	for i=0,2 do
		SendKnife("KnifeLeftDeadly",{300,(64-dist/2-2*dist*i)*rand},{-4.5,0})
	end
	rand=-rand
	local dist=128/6
	for i=0,2 do
		SendKnife("KnifeRightDeadly",{-300,(64-dist/2-2*dist*i)*rand},{4.5,0})
	end
end
for i=1,#rain do
	local k=rain[i]
...
if bullet.GetVar("damage") ~= nil then
	KnifeHurt(bullet.GetVar("damage"))
else
	KnifeHurt(1)
end
end