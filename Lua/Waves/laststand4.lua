...
--------------------------------------------------------
		...
		if spawntimer%3 == 0 then
			local knife = CreateProjectile('KnifeDownLethal', -10+0.485*spawntimer, 320*orientknife)
			knife.sprite.rotation = 90 - orientknife*90
			knife.SetVar("bulletType", "knife")
			orientknife = -orientknife
			table.insert(knives, knife)
		end
		if spawntimer%35 == 0 and spawntimer<470 then
			local knife_y = math.random(60)-30
			local hknife = CreateProjectile('KnifeLeftLethal', 700, knife_y)
			hknife.SetVar("bulletType", "knife")
			table.insert(horizknives, hknife)
		end
		if spawntimer%40 == 0 then
			Audio.PlaySound("slice2")
			local vzmah = CreateProjectile('chara/att0', 0.55*spawntimer+104, 10)
