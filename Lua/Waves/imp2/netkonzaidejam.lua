...
function SendKnife(point)
	local x=7
	local k=CreateProjectile("KnifeDown"..name4,point[1],300)
	k.SetVar("speed",{0,-x})
	k.SetVar("time",0)
	k.sprite.alpha=0
	table.insert(knifes,k)
	k=CreateProjectile("KnifeUp"..name4,point[1],-300)
	k.SetVar("speed",{0,x})
	k.SetVar("time",0)
	k.sprite.alpha=0
	table.insert(knifes,k)
	k=CreateProjectile("KnifeRight"..name4,-300,point[2])
	k.SetVar("speed",{x,0})
	k.SetVar("time",0)
	k.sprite.alpha=0
	table.insert(knifes,k)
	k=CreateProjectile("KnifeLeft"..name4,300,point[2])
	k.SetVar("speed",{-x,0})
	k.SetVar("time",0)
	k.sprite.alpha=0
	table.insert(knifes,k)
...
if spawntime%60==30 then
	SendKnife({37.5,37.5})
	SendKnife({-37.5,-37.5})
	elseif spawntime%60==0 then
	SendKnife({-12.5,-12.5})
	SendKnife({12.5,12.5})
end
for i=1,#knifes do
