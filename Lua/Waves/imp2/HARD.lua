...
if spawntime%18==0 then
	local knife=CreateProjectile("KnifeDown"..name4,-90+20*math.random(8),200)
...
if spawntime%150==2 then
	napr=math.random(2)*2-3
	upi=(math.random(2)*2-3)*12.5+12.5
	elseif spawntime%150==45 then
	for i=1,4 do
		local knife=CreateProjectile("KnifeLeft"..name4,112*napr,-112.5+50*i-upi)
...
function OnHit(k)
	if k.GetVar("color") == "gold" then
		KnifeHurt(2)
		Effects("curse",1)
	elseif not  k.GetVar("color") then
		KnifeHurt(3)
		Effects("curse",2)
