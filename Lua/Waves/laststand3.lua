...
--------------------------------------------------------
		...
		if spawntimer%4 == 0 then
			local knife = CreateProjectile('KnifeDownDeadly', -10+0.5*spawntimer, 320*orientknife)
			knife.sprite.rotation = 90 - orientknife*90
			knife.SetVar("bulletType", "knife")
			orientknife = -orientknife
			table.insert(knives, knife)
		end
		if spawntimer%48 == 0 and spawntimer<470 then
			local knife_y = math.random(60)-30
			local hknife = CreateProjectile('KnifeLeftDeadly', 700, knife_y)
