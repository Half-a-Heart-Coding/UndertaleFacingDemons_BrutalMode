...
Moving()
	...
	if attacktimer == 45 and endgame == false then
		local knife_ru = CreateProjectileAbs('KnifeDown'..name, Player.absx, 500)
		knife_ru.sprite.rotation = 0
		knife_ru.SetVar("bulletType", "mutant")
		knife_ru.SetVar("angle", 90)
	...
	for k=1,#knives do
		...
		local move_x = 0--5*math.sin(math.pi*napravlenie/180)
		local move_y = -1---5*math.cos(math.pi*napravlenie/180)
...
function OnHit(bullet)
	...
	if bullet.GetVar('bulletType') == "mutant" then
		--LongHurt(9999999,2)
		GameOver()
		--elseif bullet.GetVar('bulletType') == "beam" then
		--KnifeHurt(4)
