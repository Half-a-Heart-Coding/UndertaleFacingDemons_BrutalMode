...
Player.hp=135
purple=false
BR=CreateProjectileAbs("system/chaos/hell",550,70)
function SpawnKnife(x,y)
        knife = CreateProjectileAbs('KnifeDownMutant', x, y)
        knife.SetVar("posx", x)
        knife.SetVar("posy", y)
        knife.SetVar("angle", 0)
		knife.SetVar("color","mutant")
		knife.SetVar("chaseTime",90)
		knife.SetVar("time",0)
...
function SpawnKnife5()
	...
	if pointy<spawny then
        ...
        knife = CreateProjectileAbs('KnifeDownLethal', spawnx, spawny)
        knife.SetVar("posx", spawnx)
        knife.SetVar("posy", spawny)
        knife.SetVar("angle", angle)
...
function SpawnKnife2()
	...
	color=1
		if color==1 then
			knife = CreateProjectileAbs('KnifeDownPurpleFatal', spawnx, spawny)
			knife.SetVar("posx", spawnx)
			knife.SetVar("posy", spawny)
			knife.SetVar("angle", angle)
...
function MoveKnife(knife)
	local xdifference = Player.x - knife.x
	local ydifference = Player.y - knife.y
	local dist = math.sqrt(xdifference*xdifference + ydifference*ydifference)
	if knife.GetVar("chaseTime")>0 then
		if knife.GetVar('xspeed')==nil then
			knife.SetVar('xspeed', 0)
		end
		if knife.GetVar('yspeed')==nil then
			knife.SetVar('yspeed', 0)
		end
		local xspeed = knife.GetVar('xspeed') / 2 + xdifference / 85
		local yspeed = knife.GetVar('yspeed') / 2 + ydifference / 85
		knife.SetVar('xspeed', xspeed)
		knife.SetVar('yspeed', yspeed)
		if Player.y < knife.y then
		...
		if knife.GetVar("color")=="red" then
			dx=dx*5
			dy=dy*5
		else
			dx=dx*4
...
--EndWave()
    ...
    if spawntimer%60 == 1 then
        SpawnKnife4()
    end

	if spawntimer==1 then
        --SpawnKnife(Arena.width+50,Arena.height+50)
		--SpawnKnife(Arena.width-50,Arena.height+50)
		--SpawnKnife(Arena.width+50,Arena.height-50)
		--SpawnKnife(Arena.width-50,Arena.height-50)
		SpawnKnife(Arena.width+100,Arena.height+100)
		SpawnKnife(Arena.width-100,Arena.height+100)
		SpawnKnife(Arena.width+100,Arena.height-100)
		SpawnKnife(Arena.width-100,Arena.height-100)
    end
