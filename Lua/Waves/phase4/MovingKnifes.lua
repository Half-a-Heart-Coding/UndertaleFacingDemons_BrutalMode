...
function DoCircle(ingo)
	local knife=CreateProjectile("KnifeLeftDeadly",-100,-100)
	knife.SetVar("time",0)
	knife.SetVar("flytime",0)
	knife.sprite.alpha=0
	table.insert(circle,knife)
	knife=CreateProjectile("KnifeLeftDeadly",-100,-100)
	knife.SetVar("time",0)
	knife.SetVar("flytime",0)
	knife.sprite.alpha=0
	table.insert(circle,knife)
...
function DoDirection()
	...
	speed={1.65*math.cos(dir*math.pi/180),1.65*math.sin(dir*math.pi/180)}
	speedysave=speedy
	speedtime=0
end
...
if spawntime==120 or spawntime==150 or spawntime==180 or spawntime==210 then
	DoDirection()
...
if spawntime<300 then
	...
	ang=ang+2.5-1.5*math.sin((spawntime-300)*math.pi/120)
	else
	ang=ang+0.75
end
