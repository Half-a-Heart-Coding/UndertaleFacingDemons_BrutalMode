...
endwave=false
aud_volume = 1
--if Encounter.GetVar("diff")==25 then
--aud_pitch = 1.35
Encounter.Call("SetMusic", "SAVE2")
...
name5="Fatal"
if Encounter.GetVar("diff")==30 or Encounter.GetVar("diff")==33 then
	name5="Mutant"
end
...
--------------------------------------------------------
		...
		if spawntimer%2 == 0 then
			local knife = CreateProjectile('KnifeDown'..name5, -10+0.5*spawntimer, 320*orientknife)
			knife.sprite.rotation = 90 - orientknife*90
			knife.SetVar("bulletType", "deadly")
			if nam5=="Mutant" then
				knife.SetVar("bulletType", "mutant")
			end
			orientknife = -orientknife
			table.insert(knives, knife)
		end
		if spawntimer%45 == 0 and spawntimer<300 and Encounter.GetVar("diff")~=33 then
			local knife_y = math.random(60)-30
			local hknife = CreateProjectile('KnifeLeft'..name5, 700, knife_y)
			hknife.SetVar("bulletType", "deadly")
			if nam5=="Mutant" then
				hknife.SetVar("bulletType", "mutant")
			end
			table.insert(horizknives, hknife)
		end
		if spawntimer%20 == 0 then
			Audio.PlaySound("slice2")
			local vzmah = CreateProjectile('chara/att0', 0.5*spawntimer+104, 10)
...
function OnHit(bullet)
	if bullet.GetVar('bulletType') == "deadly" then
		if GetGlobal("KnifeProtection") == true then
			if Player.hp > 33 and Encounter.GetVar("diff") == 17 then
				LongHurt(33,60)
		...
		KnifeHurt(1)
		elseif bullet.GetVar('bulletType') == "mutant" then
		GameOver()
	end
end