...
function SwitchKnife(dir,x,y)
	if dir == 1 then
		local bullet = CreateProjectile ("KnifeRightDeadly", x, y)
		bullet.SetVar("color", "gold")
		table.insert(knives1, bullet)
	end
	if dir == 2 then
		local bullet = CreateProjectile ("KnifeLeftDeadly", x, y)
		bullet.SetVar("color", "gold")
		table.insert(knives2, bullet)
	end
	if dir == 3 then
		local bullet = CreateProjectile ("KnifeDownDeadly", x, y)
		bullet.SetVar("color", "gold")
		table.insert(knives3, bullet)
	end
	if dir == 4 then
		local bullet = CreateProjectile ("KnifeUpDeadly", x, y)
		bullet.SetVar("color", "gold")
...
Moving()
	...
	if spawntimer%185 == 60 then
		local knive4 = math.random(6)
			if knive4 == 1 then 
				local bullet = CreateProjectile ("KnifeUp", -50, -500)
				table.insert(knives4, bullet)
			elseif knive4 == 2 then 
				local bullet = CreateProjectile ("KnifeUp", -30, -500)
				table.insert(knives4, bullet)
			elseif knive4 == 3 then 
				local bullet = CreateProjectile ("KnifeUp", 10, -500)
				table.insert(knives4, bullet)
