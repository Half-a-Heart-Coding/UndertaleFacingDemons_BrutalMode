...
function SendKnife(name,pos,speed)
	if Encounter.GetVar("diff")==31 then
		name=name.."Fatal"
	end
...
function Update() 
	...
	if spawntimer % 200 == 0 then
		if (spawntimer/200)%6<2 then
			if (spawntimer/200)%6 <= 1 and math.random(4) ~= 1 then
			...
			if (spawntimer/200)%6 <= 3 and math.random(4) ~= 1 then
				SendKnife2("KnifeUpblue",{201,-32},{-3.5,0})
			else
				SendKnife3("KnifeUpOrange",{201,-32},{-3.5,0})
			end
			if (spawntimer/200)%6==5 then
				spawntimer=spawntimer+25
			end
		end
	end
