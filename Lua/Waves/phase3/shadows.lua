require "Libraries/Playermove"
require "Libraries/hurt"
spawntime=0
shadows={}
left={}
shad_anim={{1, 0, 0}, {0.5, 0, 0},{0,0,0},{0.5,0,0}}
Arena.ResizeImmediate(200,200)
for i=1,200 do
	left[i]=CreateProjectile("KnifeRightDeadly",0,0)
	left[i].sprite.alpha=0
	left[i].SetVar("color",none)
	left[i].SetVar("togo",false)
end
num=1
Player.hp = 135

function SendShadow(name, position, speed, istrue)
	local knife=CreateProjectile(name,position[1],position[2],"Top")
	knife.SetVar("speed",speed)
	knife.SetVar("name",name)
	knife.SetVar("time",0)
	knife.sprite.alpha=0
	knife.SetVar("istrue",istrue)
	knife.SetVar("color","none")
	table.insert(shadows,knife)
	time=0
end
function AllSides()
	local rand=math.random(2)*2-3
	local name=""
	if rand==-1 then
		name="KnifeRightDeadly"
		else
		name="KnifeLeftDeadly"
	end
	for i=1,4 do
		SendShadow(name,{300*rand,-137.5+50*i},{-3*rand,0},true)
	end
	if rand==1 then
		name="KnifeRightDeadly"
		else
		name="KnifeLeftDeadly"
	end
	for i=1,4 do
		SendShadow(name,{-300*rand,-112.5+50*i},{3*rand,0},true)
	end
end
		

function Update()
if Encounter.GetVar("diff") == 24 then
	HurtUpd(69)
else
	HurtUpd(690)
end
Moving()
spawntime=spawntime+1
if spawntime==1 or spawntime==151 then
	AllSides()
	elseif spawntime>279 and spawntime<361 and spawntime%10==0 then
	local pos=(spawntime-280)/10
	pos=90-20*pos
	SendShadow("KnifeRightDeadly",{-300,pos},{6,0},true)
	SendShadow("KnifeLeftDeadly",{300,pos},{-6,0},true)
	elseif spawntime>409 and spawntime<520 and (spawntime-410)%25==0 then
	local pos=(spawntime-410)/25
	pos=pos*20+10
	if pos==10 then
		SendShadow("KnifeDownDeadly",{-pos,300},{0,-5},false)
		SendShadow("KnifeDownDeadly",{pos,300},{0,-5},false)
		else
		SendShadow("KnifeDownDeadly",{-pos,300},{0,-5},true)
		SendShadow("KnifeDownDeadly",{pos,300},{0,-5},true)
	end	
	elseif spawntime>530 and spawntime<561 and spawntime%10==0 then
	local pos=spawntime/10-54
	pos=10+20*pos
	SendShadow("KnifeUpDeadly",{-pos,-300},{0,6},true)
	SendShadow("KnifeUpDeadly",{pos,-300},{0,6},true)
end

for i=1,#shadows do
	local s=shadows[i]
	if s.isactive then
		--s.sprite=(s.GetVar("name"))
		local time=s.GetVar("time")+1
		s.SetVar("time",time)
		local speed=s.GetVar("speed")
		s.Move(speed[1],speed[2])
		if time<31 then
			s.sprite.alpha=math.sin(time*math.pi/60)
			elseif (time-31)%10==0 and s.GetVar("istrue")==true then
			if s.GetVar("name") == "KnifeLeftDeadly" then
				 left[num].sprite.Set("ShadowLeft")
			elseif s.GetVar("name") == "KnifeRightDeadly" then
				 left[num].sprite.Set("ShadowRight")
			elseif s.GetVar("name") == "KnifeUpDeadly" then
				 left[num].sprite.Set("ShadowUp")
			elseif s.GetVar("name") == "KnifeDownDeadly" then
				 left[num].sprite.Set("ShadowDown")
			else
				--left[num].sprite.Set("Shadow"..s.GetVar("name"))
			end
			--s.sprite=(s.GetVar("name"))
			left[num].MoveTo(s.x,s.y)
			left[num].SetVar("time",0)
			s.SetVar("color","knife2")
			left[num].SetVar("color","knife")
			left[num].SetVar("togo",true)
			num=num+1
			if num==201 then
				num=1
			end
		end
		if s.x>400 or s.x<-400 or s.y>300 or s.y<-300 then
			s.Remove()
		end
	end
end

for i=1,#left do
	local s=left[i]
	if s.GetVar("togo")==true then
		local time=s.GetVar("time")+1
		s.SetVar("time",time)
		if time<41 then
			local sin=0.75*math.cos(time*math.pi/80)
			s.sprite.color={1,1,1}
			s.sprite.alpha=sin
			if time==20 then
				s.SetVar("color","none")
			end
			else
			s.SetVar("togo",false)
		end
	end
end




end

function OnHit(ingo)
local c=ingo.GetVar("color")
if c=="knife" then
	KnifeHurt(1)
elseif c=="knife2" then
	KnifeHurt(2)
end
end