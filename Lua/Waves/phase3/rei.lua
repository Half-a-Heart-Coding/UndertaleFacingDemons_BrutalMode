...
function SendKnife(side)
	if side==1 or side==3 then
		...
		local knife=CreateProjectile("KnifeDownDeadly",-80+20*math.random(7),200*from)
		...
		local from=math.random(2)*2-3
		local knife=CreateProjectile("KnifeLeftDeadly",200*from,-80+20*math.random(7))
		knife.SetVar("color","knife")
		knife.sprite.rotation=90*(from-1)
...
if attacks.GetVar("color") == "asriel" then
   if Player.hp<11 and Player.hp > 1 then
      LongHurt(Player.hp-1,30)
      else LongHurt(10,20)
   end
