...
alltimer=alltimer+1
  if (spawntime % 75 == 60 or spawntime % 75 == 65) and Phase1  then 
	if spawntime % 75 == 60 then
		knife(35*t,beg,"white","KnifeDown"..name3,1)
		--knife(40*t,beg,"blue","KnifeDownblue"..name3,1)
		else
		i=i+1
		knife(50*t,beg,"blue","KnifeDownblue"..name3,1)
		t=-t   
	end
  end
  if (spawntime%75)<57 and spawntime%5==0 and Phase1 then
	knife((20*(40-spawntime%75))*-t/90*8,beg,"white","KnifeDown"..name3,1)
end
if death==false then
...
if (spawntime%80>77 or spawntime%80<3) then
	knife(-300,32,"blue","KnifeDownblue"..name3,2)
	knife(-300,-32,"blue","KnifeUpblue"..name3,2)
     r=math.random(2)*2-3
	elseif spawntime%80==3 then
	n=n+1
     elseif spawntime%80<29 and spawntime%80>21  then
	knife(-300,-36*r,"red","KnifeUp"..name4,2,(r-1)*90)
     elseif spawntime%80<59 and spawntime%80>51 then
	knife(-300,36*r,"red","KnifeDown"..name4,2,(r-1)*90)
end
end
...
if Phase4 and death==false then
  if spawntime%120==95 then
    r=math.random(2)*2-3
	if n > 0 then
		local knife = CreateProjectile ("laser0/laserv0",95,0)
		Audio.PlaySound("bolt")
		table.insert(beam,knife)
	end
    elseif spawntime%120==98 then
	knife(-320,-36*r,"death_knife","KnifeUp"..name5,3,(r-1)*90)
	...
	hit = CreateProjectile ("att0",0,10*r)
    angle=-90
    hit.SetVar("angle")
    hit.sprite.rotation=angle
    hit.SetVar("color","none")
    ...
    Audio.PlaySound("bolt")
    table.insert(beam,knife)
	knife = CreateProjectile ("laser0/laserh0",0,-30*r)
    Audio.PlaySound("bolt")
    table.insert(beam,knife)
	...
	n=n+1
	local hit = CreateProjectile ("att0",95,0)
    angle=0
    hit.SetVar("angle")
    hit.sprite.rotation=angle
    hit.SetVar("color","none")
