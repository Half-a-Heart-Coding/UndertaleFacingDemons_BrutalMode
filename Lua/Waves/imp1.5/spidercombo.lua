...
if spawntime==1 then
	spider={}
	for i=1,4 do
		spider[i]=CreateProjectile("spider",88-176*(i%2),-60+25*i)
		spider[i].SetVar("time",0)
		spider[i].SetVar("side",-spider[i].x/88)
		spider[i].SetVar("px",0)
		spider[i].SetVar("type","spider")
		lines[i]=CreateProjectile("purplepoint",100-200*(i%2),-60+25*i)
...
if spawntime>150 and (spawntime-150)%40==0 then
	...
	local spid=CreateProjectile("spider",330*sided,((liner-2)*25)-12.5)
	sided=-sided
	table.insert(spider,spid)
	spid.SetVar("type","spider")
	spid.SetVar("px",5*sided)
end
...
if spawntime>150 and (spawntime-150)%200==0 and #lines>1 then
...
for i=1,#bombs do
	...
	if b.isactive then
		...
		if time==0 then
			b.Move(0,-3.75)
			if (b.y<41 and #lines==3) or (b.y<17 and #lines==2) or (b.y<66 and #lines==4) then
				b.SetVar("time",1)
			end
			else
			...
			if time<20 then
				...
				elseif time==20 then
				if b.y<66 and #lines==4 then
					EraseLine(4)
				elseif b.y<41 and #lines==3 then
					EraseLine(3)
...
for i=1,#spears do
	...
	if s.isactive then
		...
		if l==0 and s.y<57+12.5 then
			s.SetVar("level",1)
			DoHole(s.x,4)
			elseif s.y<32+12.5 and l==1 then
			s.SetVar("level",2)
			DoHole(s.x,3)
			elseif s.y<7+12.5 and l==2 then
			s.SetVar("level",3)
			DoHole(s.x,2)
			elseif s.y<-18+12.5 and l==3 then
			s.SetVar("level",4)
			DoHole(s.x,1)
		end
		if s.y<-400 then
...
function OnLines()
	local oldline=line
	if Input.Up==1 and line<4 then
		line=line+1
		elseif Input.Down==1 and line>1 then
		line=line-1
	end
	CheckPosition({Player.x,((line-2)*25)-12.5},{Player.x,Player.y})
	local px=0
...
function DoHole(px,lin)
	if lin<#lines+1 then
		local Hole=CreateProjectile("purplepoint",px,((lin-2)*25)-12.5)
