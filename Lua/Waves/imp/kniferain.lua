Arena.resize(300,300)
collision = require "Libraries/rotational_collision"
require "Libraries/Playermove"
require "Libraries/hurt"
attack={}

DoGhost("smile",0,600)

name2=""
if GetGlobal("DifficultyPoints") >= 621 then
	name2="Fatal"
end
name3="Deadly"
if GetGlobal("DifficultyPoints") >= 621 then
	name3="Fatal"
end
name4="Lethal"
if GetGlobal("DifficultyPoints") >= 621 then
	name4="Fatal"
end

kniferainleft={}
kniferainright={}
spawntime=0
loop=80
life={}
right={}
up={}
pose=0
rand=0
away=210
rew={}
timer=0
limit=100
speed1=10
speed2=15
speedup1={}
speedup2={}
knives={}
targetattack={}
function Update()
HurtUpd(900)
Moving()
spawntime=spawntime+1
timer=timer+1
if spawntime%30==0 then
  pose=math.random(20)
  local knife = CreateProjectile("KnifeRight"..name3,-400,-150+15*pose)
  --knife.SetVar("color", "red")
  table.insert( kniferainleft , knife)
end
if spawntime%30==15 then
  pose=math.random(20)
  local knife = CreateProjectile("KnifeLeft"..name3,400,-150+15*pose)
  --knife.SetVar("color", "red")
  table.insert( kniferainright , knife)
end    
for g=1,#kniferainright do
  local bullet = kniferainright[g]
  if bullet.x<115 then
    speedup1[g]=speed1
    else speedup1[g]=speed1/2
  end
	if (Player.x+38>bullet.x and Player.x-38<bullet.x) and (Player.y+16>bullet.y and Player.y-16<bullet.y) then
		KnifeHurt(1)
	end
  bullet.MoveTo(bullet.x-speedup1[g],bullet.y)
end    
for n=1,#kniferainleft do
  local bullet = kniferainleft[n]
  if bullet.x>-115 then
    speedup2[n]=speed1
    else speedup2[n]=speed1/2
  end
	if (Player.x+38>bullet.x and Player.x-38<bullet.x) and (Player.y+16>bullet.y and Player.y-16<bullet.y) then
		KnifeHurt(1)
	end

  bullet.MoveTo(bullet.x+speedup2[n],bullet.y)
end
  if timer==limit then
    timer=timer-limit
    limit=limit-3
    if timer>30 then
      timer=timer-3
    end
    rand= math.random(2)
    if rand==1 then
      local knive1 = CreateProjectile("KnifeDown"..name3,Player.x,Player.y+away)
      local knive2 = CreateProjectile("KnifeUp"..name3,Player.x,Player.y-away)
      local knive3 = CreateProjectile("KnifeLeft"..name3,Player.x+away,Player.y)
      local knive4 = CreateProjectile("KnifeRight"..name3,Player.x-away,Player.y)
	knive1.sprite.alpha=0
	knive2.sprite.alpha=0
	knive3.sprite.alpha=0
	knive4.sprite.alpha=0
      table.insert(targetattack,knive1)
      table.insert(targetattack,knive2)
      table.insert(targetattack,knive3)
      table.insert(targetattack,knive4)
      else 
      local knive1 = CreateProjectile("KnifeDown"..name3,Player.x+away,Player.y+away)
      local knive2 = CreateProjectile("KnifeDown"..name3,Player.x+away,Player.y-away)
      local knive3 = CreateProjectile("KnifeDown"..name3,Player.x-away,Player.y+away)
      local knive4 = CreateProjectile("KnifeDown"..name3,Player.x-away,Player.y-away)
	knive1.sprite.alpha=0
	knive2.sprite.alpha=0
	knive3.sprite.alpha=0
	knive4.sprite.alpha=0
      table.insert(targetattack,knive1)
      table.insert(targetattack,knive2)
      table.insert(targetattack,knive3)
      table.insert(targetattack,knive4)
      knive1.sprite.rotation=-45
      knive2.sprite.rotation=-135
      knive3.sprite.rotation=45
      knive4.sprite.rotation=135
    end
  end
for i=1,#targetattack do
  local bullet = targetattack[i]
  if not rew[i]  then
    if (Player.x-bullet.x)>200 and (Player.x-bullet.x)<220 then
      right[i]=1
      elseif (Player.x-bullet.x)>-220 and (Player.x-bullet.x)<-200 then
      right[i]=-1 
      else right[i]=0
    end
    if (Player.y-bullet.y)>200 and (Player.y-bullet.y)<220 then
      up[i]=1
      elseif (Player.y-bullet.y)>-220 and (Player.y-bullet.y)<-200 then
      up[i]=-1 
      else up[i]=0
    end
    rew[i]=43
    elseif rew[i]>12 then
      rew[i]=rew[i]-1
	bullet.Move(right[i],up[i])
	local t=42-rew[i]
	bullet.sprite.alpha=math.sin(t*math.pi/60)
    elseif rew[i]>0 and rew[i]<13 then
    rew[i]=rew[i]-1
    bullet.MoveTo(bullet.x+speed2*right[i],bullet.y+speed2*up[i])
  end
	if (Player.x+40>bullet.x and Player.x-40<bullet.x) and (Player.y+40>bullet.y and Player.y-40<bullet.y) then
		if collision.CheckCollision(Player, bullet) then
			KnifeHurt(2)
		end
	end
end
end

function OnHit(attacks)
end