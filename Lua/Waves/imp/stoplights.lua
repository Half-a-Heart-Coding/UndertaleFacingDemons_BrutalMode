...
if spawntime%50==0 then
   local knife = CreateProjectile("KnifeDown"..name,Player.x,64)
...
if spawntime%100==25 then
   ...
   local knife = CreateProjectile("knifeLeft"..name,182*rand2,Player.y)
   table.insert(knifesfly,knife)
   knife.SetVar("color","white")
	knife.SetVar("time",0)
   knife.sprite.rotation=90-rand2*90
end
if spawntime%66==0 or spawntime%66==15 or spawntime%66==30 or spawntime%66==45 then
   if spawntime%66==0 then
      local knife = CreateProjectile("speardown"..name,-350,0)
      knife.SetVar("dir",3)
      knife.sprite.color={0/255, 168/255, 242/255}
      knife.SetVar("color","blue") 
      knife.SetVar("speed",3)
	knife.SetVar("time",0)
      table.insert(knifes1,knife)
	elseif spawntime%66==15 then
      local knife = CreateProjectile("speardown"..name,350,0)
      knife.SetVar("dir",-3)
      knife.sprite.color={0/255, 168/255, 242/255}
      knife.SetVar("color","blue") 
	...
	knife.SetVar("time",0)
      table.insert(knifes1,knife)
	 elseif spawntime%66==30 then
	 local knife = CreateProjectile("speardown"..name,-350,0)
      knife.sprite.rotation=180
      knife.SetVar("dir",3)
      knife.sprite.color={255/255, 140/255, 0/255}
      knife.SetVar("color","orange")
      knife.SetVar("speed",3)
	knife.SetVar("time",0)
      table.insert(knifes1,knife)
    else local knife = CreateProjectile("speardown"..name,350,0)
      knife.sprite.rotation=180
      knife.SetVar("dir",-3)
      knife.sprite.color={255/255, 140/255, 0/255}
      knife.SetVar("color","orange")
