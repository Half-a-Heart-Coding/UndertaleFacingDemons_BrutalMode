...
if spawntime%72==36 then
	local knife=CreateProjectile("KnifeLeft"..name2,68,100)
	table.insert(knifes,knife)
	elseif spawntime%72==57 then
	local knife=CreateProjectile("KnifeRight"..name2,-68,100)
	table.insert(knifes,knife)
end
if spawntime>549 and spawntime<651 then
	...
	if spawntime==620 then
		for i=1,5 do
			local knife1=CreateProjectile("KnifeUp"..name2,i*20-10-400,-68)
			knife1.SetVar("point",i*20-10)
			knife1.SetVar("side",-1)
			knife1.SetVar("time",0)
			table.insert(knifes2,knife1)
			local knife2=CreateProjectile("KnifeUp"..name2,i*-20+10+500,-68)
...
if spawntime%60==0 then
	...
	if rand==1 then
		SendKnife({-200,68},{2,0},"KnifeDown"..name2,160)
		--elseif rand==2 then
		--SendKnife({200,-68},{-2,0},"KnifeUp"..name2,160)
		elseif rand==3 then
		SendKnife({68,-200},{0,2},"KnifeLeft"..name2,160)
		else
		SendKnife({-68,200},{0,-2},"KnifeRight"..name2,160)
	end
...
for i=1,#knifes3 do
	...
	if k.isactive then
		...
		if k.x+(k.sprite.width/2+4)>Player.x and k.x-(k.sprite.width/2+4)<Player.x and k.y+(k.sprite.height/2+4)>Player.y and k.y-(k.sprite.height/2+4)<Player.y then
			KnifeHurt(1)
		end
	end
end
