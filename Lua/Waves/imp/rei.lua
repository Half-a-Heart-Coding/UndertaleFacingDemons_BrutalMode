...
function SendKnife(side)
	if side==1 or side==3 then
		...
		local knife=CreateProjectile("KnifeDown"..name3,-80+20*4,200*1)--math.random(7) replace 4
		knife.SetVar("color","knife")
		knife.sprite.rotation=90*(1-1)
		knife.SetVar("speed",{0,-10*1})
		knife.SetVar("stars",{10*(side-2),0})
		knife.SetVar("time",0)
		table.insert(knifes,knife)
		elseif side==2 or side==4 then
		local from=math.random(2)*2-3
		local knife=CreateProjectile("KnifeLeft"..name3,200*1,-80+20*4)--math.random(7) replace 4
		knife.SetVar("color","knife")
		knife.sprite.rotation=90*(1-1)
		knife.SetVar("speed",{-10*1,0})
		knife.SetVar("stars",{0,10*(side-3)})
		knife.SetVar("time",0)
		table.insert(knifes,knife)
	end
end
