...
spriteName = "vert_knife"
if GetGlobal("DifficultyPoints") >= 621 then
	spriteName = "KnifeDownFatal"
end
endgame = false
sdvig = 1
...
Moving()
	...
	if attacktimer > 55 and attacktimer < 75 and endgame == false then
	...
	if attacktimer == 90 and endgame == false then
			...
			local knife_ru = CreateProjectileAbs(spriteName, 630, 470)
			knife_ru.sprite.rotation = -10
			knife_ru.SetVar("bulletType", "knife")
			knife_ru.SetVar("angle", -10)
			table.insert(knives, knife_ru)

			local knife_rb = CreateProjectileAbs(spriteName, 650, 10)
			knife_rb.sprite.rotation = 192
			knife_rb.SetVar("bulletType", "knife")
			knife_rb.SetVar("angle", 192)
			table.insert(knives, knife_rb)

			local knife_lb = CreateProjectileAbs(spriteName, 40, 10)
			knife_lb.sprite.rotation = 170
			knife_lb.SetVar("bulletType", "knife")
			knife_lb.SetVar("angle", 170)
			table.insert(knives, knife_lb)

			local knife_lu = CreateProjectileAbs(spriteName, 20, 470)
			knife_lu.sprite.rotation = 15
			knife_lu.SetVar("bulletType", "knife")
			knife_lu.SetVar("angle", 15)
			table.insert(knives, knife_lu)
	elseif attacktimer == 950 and endgame == false then
			local knife_rm = CreateProjectileAbs(spriteName, 600, 500)
			knife_rm.sprite.rotation = 91
			knife_rm.SetVar("bulletType", "knife")
			knife_rm.SetVar("angle", 91)
			table.insert(knives, knife_rm)

			local knife_um = CreateProjectileAbs(spriteName, 300, 0)
			knife_um.sprite.rotation = 181
			knife_um.SetVar("bulletType", "knife")
			knife_um.SetVar("angle", 181)
			table.insert(knives, knife_um)

			local knife_lm = CreateProjectileAbs(spriteName, 0, 500)
			knife_lm.sprite.rotation = 271
			knife_lm.SetVar("bulletType", "knife")
			knife_lm.SetVar("angle", 271)
			table.insert(knives, knife_lm)

			local knife_bm = CreateProjectileAbs(spriteName, 300, 1000)
			knife_bm.sprite.rotation = 1
			knife_bm.SetVar("bulletType", "knife")
			knife_bm.SetVar("angle", 1)
			table.insert(knives, knife_bm)
	end
	...
	if attacktimer==390 then
		DoWarns(20,1)
		DoWarns(50,2)
	elseif attacktimer==420 then
		DoWarns(45,2)
		DoWarns(25,1)
	end

	if attacktimer==150 then
		DoWarns(0,1)
		DoWarns(0,2)
	elseif attacktimer==300 then
		DoWarns(45,2)
		DoWarns(45,1)
	end

	if attacktimer==450 then
		DoWarns(20,2)
		DoWarns(50,1)
	elseif attacktimer==460 then
		DoWarns(45,1)
		DoWarns(25,2)
	end

	if attacktimer==525 then
		DoWarns(5,1)
		DoWarns(50,1)
	elseif attacktimer==530 then
		DoWarns(5,2)
		DoWarns(55,2)
	end

	if attacktimer == 590 then
		DoWarns(5,1)
		DoWarns(5,2)
	elseif attacktimer==621 then
		DoWarns(15,1)
		DoWarns(15,2)
	elseif attacktimer==666 then
		DoWarns(50,1)
		DoWarns(50,2)
	elseif attacktimer==700 then
		DoWarns(40,1)
		DoWarns(40,2)
	elseif attacktimer==750 then
		DoWarns(0,1)
		DoWarns(0,2)
	end

	if attacktimer % 72 == 0 then
		if (attacktimer/72)%6<2 then
			if (attacktimer/72)%6 <= 1 then
				SendKnife2("KnifeDownblue"..name,{201,32},{-3.5,0})
				local knife_ru = CreateProjectileAbs(spriteName, 630, 470)
				knife_ru.sprite.rotation = -25--(attacktimer%37 / 37 * 90) - 90
				knife_ru.SetVar("bulletType", "knife")
				knife_ru.SetVar("angle", -25)--(attacktimer%37 / 37 * 90) - 90)
				table.insert(knives, knife_ru)
			else
				SendKnife3("KnifeDownOrange"..name,{201,32},{-3.5,0})
				local knife_rb = CreateProjectileAbs(spriteName, 650, 10)
			knife_rb.sprite.rotation = 210--(attacktimer%37 / 37 * 90) + 180
			knife_rb.SetVar("bulletType", "knife")
			knife_rb.SetVar("angle", 210)--(attacktimer%37 / 37 * 90) + 180)
			table.insert(knives, knife_rb)
			end
		else
			if (attacktimer/72)%6 <= 3 then
				SendKnife2("KnifeUpblue"..name,{201,-32},{-3.5,0})
				local knife_lb = CreateProjectileAbs(spriteName, 40, 10)
			knife_lb.sprite.rotation = 155--(attacktimer%37 / 37 * 90) + 120
			knife_lb.SetVar("bulletType", "knife")
			knife_lb.SetVar("angle", 155)--(attacktimer%37 / 37 * 90) + 120)
			table.insert(knives, knife_lb)
			else
				SendKnife3("KnifeUpOrange"..name,{201,-32},{-3.5,0})
				local knife_lu = CreateProjectileAbs(spriteName, 20, 470)
			knife_lu.sprite.rotation = 25--(attacktimer%37 / 37 * 90)
			knife_lu.SetVar("bulletType", "knife")
			knife_lu.SetVar("angle", 25)--(attacktimer%37 / 37 * 90))
			table.insert(knives, knife_lu)
			end
		end
	end
	...
	--Encounter.SetVar("quichehp",99)

	if attacktimer == 1200 then
	...
	if attacktimer > 1200 then
		...
		bone.sprite.alpha=(attacktimer - 1000) / 100
...
function OnHit(bullet)
	...
	if color == "orange" and not isMoving() then
		KnifeHurt(1)
