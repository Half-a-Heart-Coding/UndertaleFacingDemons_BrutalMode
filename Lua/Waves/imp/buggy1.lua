   ...
   if rand1%2==0 then
      for i=-7,7 do
         ...
         if i-2<rand2 and i+2>rand2 then
            ...
            local pellet2 = CreateProjectile("pellet",i*10,300)
            table.insert(pellets,pellet2)
            pellet2.SetVar("dir",-rand1+2)
            pellet2.SetVar("safe","almost"..rand4)
            if rand4==1 then
                pellet2.sprite.color={0/255, 168/255, 242/255}
            else
                pellet2.sprite.color={1, 0.5, 0}
            end
      ...
      for i=-7,7 do
         ...
         if i-2<rand2 and i+2>rand2 then
            pellet.SetVar("safe","safe"..rand3)
            local pellet2 = CreateProjectile("pellet",300*(rand1-2),i*10)
            table.insert(pellets,pellet2)
            pellet2.SetVar("dir",-rand1+2)
            pellet2.SetVar("safe","almost"..rand4)
            if rand4==1 then
...
function OnHit(pellet)
    if (pellet.GetVar("safe")=="almost1" or pellet.GetVar("safe")=="almost2") then
        if pellet.GetVar("safe")=="almost1" and isMoving() then
	        LongHurt(10,60)
        elseif pellet.GetVar("safe")=="almost2" and not isMoving() then
            LongHurt(10,60)
        end
    else
        LongHurt(10,60)
