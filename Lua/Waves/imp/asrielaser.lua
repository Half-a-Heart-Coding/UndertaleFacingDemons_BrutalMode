...
Moving()
	...
	if (timer+30)%75 == 0 and timer<1100 then
	...
	if (timer+30)%90 == 0 and timer>1100 then
		h_alt = math.random(2)*2-3 -- подозреваю, что есть способ поизящнее, ноооо.....
		v_alt = math.random(2)*2-3 -- говнокооод, говнокооооод, на вид как говно, а на деле он код
		local mathRand=math.random(4)
		if mathRand == 2 then
	...
	if timer%240==1 and timer>99999 then
		...
		if timer%240<60 then
			...
			elseif timer%240==60 and timer>99999 then
			bigstar.Remove()
			stars={}
			randomangle=math.random(24)
			for i=1,8 do
			...
			elseif timer%240<251 and timer>99999 then
			local away=300*math.sin((timer%240-60)*math.pi/300)
			for i=1,6 do
				local angle=(360/#stars)*i+(timer%240-60)*2+randomangle
				...
				local x=stars[i].x
				local y=stars[i].y
				if timer%240>150 then
					stars[i].sprite.alpha=math.cos((timer%240-60)*math.pi/120)
				end
				if px+14>x and px-14<x and py+14>y and py-14<y and stars[i].sprite.alpha>0.5 then
					LongHurt(10,30)
				end
				if timer%240==239 then
					stars[i].Remove()
				end
			end
		end
